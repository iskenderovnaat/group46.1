...
class Magic(Hero):
    ...
    def apply_super_power(self, boss, heroes):
        for hero in heroes:
            if hero != self:
                hero.damage += self.boost_damage
                print(f'Magic {self.name} boosted {hero.name} by {self.boost_damage}. New damage: {hero.damage}')
class Hacker(Hero):
...
def start_game():
    ...
    witcher = Witcher(name='Geralt', health=240, damage=0)
    hacker = Hacker(name='Rown', health=220, damage=5, stolen_health=0)
